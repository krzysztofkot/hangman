...
describe("Custom commands", function () {
  beforeEach(() => {
    cy.setRespHard();
    cy.fixture("difficultWord.json").then((obj) => {
      this.word = obj.word;
    });
  });
  ...
  it("Should guess password", () => {
    cy.assertLivesLost(0, "a", "c", "e", "n", "t", "u", "r");
    cy.assertVisibleHangmanElements(0);

    cy.get(".lives__value").then(($lives) => {
      const val = parseInt($lives.text());
